...
use jito_protos::{
    auth::{
        ...
        block_engine_relayer_client::BlockEngineRelayerClient, packet_batch_update::Msg, AccountsOfInterestRequest, AccountsOfInterestUpdate, ExpiringPacketBatch, PacketBatchUpdate, ProgramsOfInterestRequest, ProgramsOfInterestUpdate, SanitizedTransactionBatchUpdate
    }, convert::{packet_to_proto_packet}, packet::PacketBatch, shared::{Header, Heartbeat}
};
use log::{error, *};
...
impl BlockEngineRelayerHandler {
    ...
    async fn start_event_loop(
        ...
        let _response = client
            .start_expiring_packet_batch_stream(ReceiverStream::new(
            ...
            select! {
                ...
                block_engine_batches = block_engine_receiver.recv() => {
                    ...
                    let block_engine_batches = match block_engine_batches {
                        Ok(block_engine_batches) => block_engine_batches,
                        Err(broadcast::error::RecvError::Lagged(n)) => {
                            warn!("Receiver lagged by {n} messages, continuing to receive future messages.");
                            continue;
                        }
                        Err(_) => {return Err(BlockEngineError::BlockEngineFailure("block engine packet receiver disconnected".to_string()));}
                    };
                    ...
                    if let Some(filtered_transactions) = filtered_transactions {
                        let now = Instant::now();
                        let packet_forward_count = Self::forward_packets(&block_engine_packet_sender, filtered_transactions).await?;
